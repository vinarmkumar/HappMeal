  ...
  try {
    ...
    if (ingredients) {
      ...
      const detailedRecipes = await Promise.allSettled(
        recipesData.slice(0, parseInt(number)).map(async (recipe) => {        try {
          ...
          try {
            ...
            throw new Error(`Failed to fetch recipe details: ${axiosError.response?.status || axiosError.message}`);
          }
  ...
  try {
    ...
    if (!isTempId && recipeData.id && userId) {
      const existingRecipe = await Recipe.findOne({
        $or: [
          { _id: recipeData.id },
          { externalId: recipeData.id.toString() }
        ]
      });
